    ...
    if request.method == "POST":
        ...
        if 'robotPicUpload' in request.files:
            ...
            if file.filename and app.allowed_file(file.filename):
                try:
                    with open("mysite/scouting/incrementUploads.txt", "r") as incrementFile:
                    ...
                    with open("mysite/scouting/incrementUploads.txt", "w") as incrementFile:
                        incrementFile.write(str(int(currentValue) + 1))
                    file.save(app.root_path + "/" +
                              app.config['UPLOAD_FOLDER'] + "/" + filename)
                except:
                    return os.path.dirname(os.path.abspath(__file__))+os.getcwd()
