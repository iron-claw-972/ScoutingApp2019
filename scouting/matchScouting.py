...
def matchScouting(request):
    ...
    if request.method == "GET":
        ...
        if('matchNumber' in fields):
            ...
            teams = {teamlist[c]: team if not team in datateams else 'a'+team for c, team in enumerate(
                teams)}
            print(teams)
        ...
        newdata = {"MatchID": '', 'Defense': '', 'Fouls': '', "TeamNumber": '', 'Climb': '', 'BuddyClimb': '', 'ClimbTime': '',
                   'TopH': '', 'MidH': '', 'LowH': '', 'CarH': '', 'CarC': '', 'LowC': '', 'MidC': '', 'TopC': '', 'Comments': '', 'Sandstorm': '', 'ScoutName': '', 'CrossedHabLine':'Did not', 'ScoredInSandstorm':'Did not'}
        ...
        try:
            newdata.update({"MatchID": session['matchid'], "BuddyClimb": (data['buddyClimbNum'][-1] if 'buddyClimb' in data else '0'), 'Fouls': (data['fouls'] if 'fouls' in data else '0'), 'Defense': (data['defenseNum'] if 'defense' in data else '0'), "TeamNumber": session['teamnumber'], 'Climb': data['climbLevel'], 'ClimbTime': data['climbTime'],
                            'TopH': hatchData[3], 'MidH': hatchData[2], 'LowH': hatchData[1], 'CarH': hatchData[0], 'CarC': cargoData[0], 'LowC': cargoData[1], 'MidC': cargoData[2], 'TopC': cargoData[3], 'Comments': data['comments'], 'Sandstorm': data['sandstorm'], 'ScoutName': data['scoutname'], 'CrossedHabLine': (data['Hab'] if 'Hab' in data else 'Did not'), 'ScoredInSandstorm': (data['sscore'] if 'sscore' in data else 'Did not')})
        except IndexError:
